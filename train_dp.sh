#!/bin/sh
#SBATCH --job-name=train_dp_cup
#SBATCH -p preempt
#SBATCH --nodes=1
#SBATCH -A marlowe-m000073
#SBATCH -G 2
#SBATCH --cpus-per-task=32
#SBATCH --error=outputs/test/train_dp.err
#SBATCH --output=outputs/test/train_dp.out
...
command="$accelerate_path launch --num_processes 2 train.py --config-name=train_diffusion_unet_timm_umi_workspace task.dataset.zarr_path=/dev/shm/uva/umi_data/cup_arrangement_0.zarr"
echo $command
exec $command
