...
def compress_data(dataset_name: str, data_dir: str, output_dir: str):
    ...
    if os.path.exists(f"{output_dir}/{dataset_name}.zarr.tar.lz4"):
        print(f"Skipping {dataset_name} because it already exists in {output_dir}")
        return
    print(f"Compressing {data_dir}/{dataset_name}.zarr to {output_dir}/{dataset_name}.zarr.tar.lz4")
    subprocess.run([f"tar cf - {dataset_name}.zarr | lz4 -c > {output_dir}/{dataset_name}.zarr.tar.lz4"], cwd=data_dir, shell=True, check=True)
...
@click.option("--output_dir", type=str, default="/dev/shm/uva/umi_data/zarr")
