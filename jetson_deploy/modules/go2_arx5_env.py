...
from jetson_deploy.modules.go2_arx5_controller import Go2Arx5Controller
...
class Go2Arx5Env:
    ...
    def __init__(self, 
        ...
        robots: List[Go2Arx5Controller] = list()


        for rc in robots_config:
            this_robot = Go2Arx5Controller(
                shm_manager=shm_manager,
                verbose=True
            )
    ...
    def exec_actions(
        ...
        # Does not deal with robot latency. The timestamps are based on the observation input
        assert self.is_ready
        ...
        for robot_idx, (robot, rc) in enumerate(zip(self.robots, self.robots_config)):
            robot.send_trajectory(
                pose_traj=new_actions[:, robot_idx*7:robot_idx*7+6],
                gripper_traj=new_actions[:, robot_idx*7+6],
